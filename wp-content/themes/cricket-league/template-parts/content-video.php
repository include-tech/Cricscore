...
<?php
  $cricket_league_content = apply_filters( 'the_content', get_the_content() );
  $cricket_league_video = false;

  // Only get video from the content if a playlist isn't present.
  if ( false === strpos( $cricket_league_content, 'wp-playlist-script' ) ) {
    $cricket_league_video = get_media_embedded_in_content( $cricket_league_content, array( 'video', 'object', 'embed', 'iframe' ) );
  }
?>
  ...
  <div class="post-main-box p-3 mb-3 wow zoomIn" data-wow-duration="2s">
    <?php
      if ( ! is_single() ) {
        ...
        if ( ! empty( $cricket_league_video ) ) {
          foreach ( $cricket_league_video as $cricket_league_video_html ) {
            echo '<div class="entry-video">';
              echo $cricket_league_video_html;
            echo '</div>';
          }
        };
      };
